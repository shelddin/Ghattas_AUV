...
class sway_left_for(object):
    ...
    def __init__(self):
        ...
        self.rc_publisher = rospy.Publisher('/mavros/rc/override', OverrideRCIn,
                                            queue_size=10)
        self.service_server_object = rospy.Service('/autonomous/sway_left_for',
                                                   dist, self.service_callback)
        rospy.loginfo("The sway_left_for service is ready ")
...
if __name__ == '__main__':
    ...
    rospy.init_node('sway_left_for_serviceServer')
    object__ = sway_left_for()
    rospy.spin()
