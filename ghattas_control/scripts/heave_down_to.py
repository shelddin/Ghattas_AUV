...
class heave_down_to(object):
    def __init__(self):
        self.depth_tools_object = depth_tools()
        self.rc_publisher = rospy.Publisher('/mavros/rc/override', OverrideRCIn,
                                            queue_size=10)
        self.service_server_object = rospy.Service('/autonomous/heave_down_to',
                                                   depth, self.service_callback)
        rospy.loginfo("The heave_down_to service is ready ")
...
if __name__ == '__main__':
    rospy.init_node('heave_down_to_serviceServer')
    object__ = heave_down_to()
    rospy.spin()
